...
#
# Ao acrescentar um motivo aqui, acrescente também o texto correspondente em
# `MotivoVinculo.tsx`. Sem isso o motivo novo aparece cru na tela do usuário e
# nada falha — não há erro de compilação ligando os dois arquivos.
MOTIVOS_VINCULO = {
...
# reais: "Helena Quirino" tinha "Otávio Quadros" como melhor palpite (0.28), e
# "Anderson Vasques" tinha "Alderson Campos" (0.55). Vínculo errado é pior que
# vínculo ausente.
...
def _e_reuniao_interna(nome_cliente: str) -> bool:
    ...
    menos dois. Sem isso, "Roberto Amaral Pinto" seria recusado por começar
    com "Roberto", e "Marina Costa Vieira" por terminar com um sobrenome comum —
...
def _casa_token(a: str, b: str) -> bool:
    ...
    Aceita prefixo a partir de 3 letras para cobrir apelido — "Ros" por
    "Rosana". Prefixo, e não substring: "ros" casa com "rosana" e com
    "roseli", mas não com "barros", que é sobrenome e não o nome de ninguém
    chamada Ros.
    """
    if a == b:
        return True
...
def nome_confiavel_para_criar(nome: str, titulo: Optional[str]) -> bool:
    ...
    Foi "Paulo Henrique Duarte Rocha" x "Paulo Henrique Sampaio" que
    ...
    casos é "Rocha" contra "Sampaio", um token que se contradiz.
    ...
    Com um token só, nunca cria: "Marcelo" não autoriza escrever no negócio do
    Marcelo Paulo Antunes.
    """
...
async def desanexar_transcricao_do_crm(briefing_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # estado em que alguns registros ficaram após uma limpeza: sem atividade
...
def _filtro_busca_cliente(busca: str) -> Optional[str]:
    ...
    gravado só com dígitos — buscar "000.000.000-00" literalmente nunca acha
