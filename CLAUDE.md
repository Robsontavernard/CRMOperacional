...
> **O que não está neste repositório.** Ele foi publicado limpo, sem histórico, a
> partir do repositório de desenvolvimento. Ficaram de fora os arquivos
> `backend/test_*.py` e o protótipo `SUSEP/` — então as referências a eles neste
> documento descrevem o que foi medido, não algo que você consegue rodar aqui.
> Os nomes de cliente nos exemplos são fictícios, trocados por nomes que
> preservam a propriedade linguística de cada caso (mesma colisão de prefixo,
> mesmo token que se contradiz); as medições e as contagens são reais.
...
- Nome casa por **palavra inteira**, com prefixo a partir de 3 letras para apelido (`Ros`→`Rosana`, `Bea`→`Beatriz`). **Nunca volte a usar substring do texto corrido**: `"ros"` casava com "Marina **Ros**eli" e "Bar**ros**", e 7 de 24 transcrições eram decididas pela ordem em que a API devolvia.
...
- **Quem conduz nunca é cliente** (`CONDUTORES`, motivo `REUNIAO_INTERNA`). Numa
  ...
  **subconjunto de tokens com pelo menos dois** — recusar por primeiro nome
  derrubaria "Roberto Amaral Pinto", e por sobrenome, "Marina Costa Vieira".
